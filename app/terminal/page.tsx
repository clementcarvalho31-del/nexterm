  ...
  return (
    <div ref={ref} onClick={() => onEnter(m.id)}
      ...
      style={{
        ...
        padding:'22px 20px', borderRadius:14, cursor:'pointer', display:'flex', flexDirection:'column', height:'100%',
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:5, marginBottom:14 }}>
        {m.stats.map((s:any) => (
          <div key={s.label} style={{ padding:'7px 5px', borderRadius:7, background:'rgba(255,255,255,.04)', border:'0.5px solid rgba(255,255,255,.06)', textAlign:'center' }}>
            <div style={{ fontSize:7, color:'#3d5060', letterSpacing:'.4px', marginBottom:2, textTransform:'uppercase' as const }}>{s.label}</div>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingTop:14, borderTop:`0.5px solid ${hov ? m.border : 'rgba(255,255,255,.05)'}`, transition:'border-color 200ms' }}>
        <span style={{ fontSize:12, fontWeight:700, color: hov ? m.color : '#8a9db5', transition:'color 200ms' }}>
          {lang === 'fr' ? 'Ouvrir le module' : 'Open module'}
        </span>
      ...
      <div style={{ position:'relative', zIndex:1 }}>
          ...
          <section style={{ padding:'40px 5vw 60px', textAlign:'center' }}>
            <div style={{ maxWidth:540, margin:'0 auto', padding:'40px', borderRadius:20, background:'rgba(240,180,41,.05)', border:'1px solid rgba(240,180,41,.18)', position:'relative', overflow:'hidden' }}>
              ...
              <button onClick={() => onEnter('calendar')} style={{ padding:'12px 32px', borderRadius:8, background:'linear-gradient(135deg,#f0b429,#d4780a)', color:'#000', fontSize:13, fontWeight:700, border:'none', cursor:'pointer', boxShadow:'0 4px 20px rgba(240,180,41,.35)', transition:'all 160ms', fontFamily:'inherit' }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform='translateY(-2px)'; (e.currentTarget as HTMLElement).style.boxShadow='0 8px 32px rgba(240,180,41,.45)' }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform='none'; (e.currentTarget as HTMLElement).style.boxShadow='0 4px 20px rgba(240,180,41,.35)' }}>
        ...
        <Reveal>
          <section style={{ padding:'0 5vw 60px', display:'flex', flexDirection:'column', alignItems:'center', gap:14 }}>
            <div style={{ fontSize:9, fontWeight:700, letterSpacing:'1.5px', color:'#2d3f50', textTransform:'uppercase' as const }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:8, justifyContent:'center' }}>
              {SECONDARY.map(item => (
                <button key={item.tab} onClick={() => onEnter(item.tab)} style={{ padding:'7px 16px', borderRadius:7, fontSize:11, fontWeight:500, color:'#4a5e72', background:'transparent', border:'0.5px solid rgba(255,255,255,.08)', cursor:'pointer', transition:'all 150ms', fontFamily:'inherit' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color='#c8d6e5'; (e.currentTarget as HTMLElement).style.borderColor='rgba(255,255,255,.18)'; (e.currentTarget as HTMLElement).style.background='rgba(255,255,255,.04)' }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color='#4a5e72'; (e.currentTarget as HTMLElement).style.borderColor='rgba(255,255,255,.08)'; (e.currentTarget as HTMLElement).style.background='transparent' }}>
                  {item[lang]}
                </button>
              ))}
            </div>
        ...
        <footer style={{ padding:'28px 5vw', borderTop:'0.5px solid rgba(255,255,255,.05)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <div style={{ width:22, height:22, background:'linear-gradient(135deg,#f0b429,#d4780a)', borderRadius:5, display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:800, color:'#000' }}>P</div>
