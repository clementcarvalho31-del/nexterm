      ...
      <div style={{ width:'100%', maxWidth:400, position:'relative', zIndex:1 }}>
        ...
        <div style={{ background:'rgba(255,255,255,.04)', border:'0.5px solid rgba(255,255,255,.1)', borderRadius:14, padding:28, backdropFilter:'blur(20px)' }}>
          <form onSubmit={handleContinue} style={{ display:'flex', flexDirection:'column', gap:14 }}>
            <div>
              <label style={{ display:'block', fontSize:12, fontWeight:600, color:'#8a9db5', marginBottom:6 }}>Email address</label>
            ...
            {step === 'password' && (
              <div>
                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                  <label style={{ fontSize:12, fontWeight:600, color:'#8a9db5' }}>Password</label>
            ...
            <button type="submit" disabled={loading} style={{ padding:'12px 20px', borderRadius:8, background:loading?'rgba(240,180,41,.6)':'linear-gradient(135deg,#f0b429,#d4780a)', border:'none', color:'#000', fontSize:14, fontWeight:700, cursor:loading?'wait':'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8, boxShadow:'0 2px 12px rgba(240,180,41,.25)' }}>
              {loading ? <><span style={{ width:14, height:14, borderRadius:'50%', border:'2px solid rgba(0,0,0,.3)', borderTopColor:'#000', animation:'t-spin .7s linear infinite', display:'inline-block' }}/> Signing in…</> : step === 'email' ? 'Continue →' : 'Sign in →'}
            </button>
          </form>
        </div>
