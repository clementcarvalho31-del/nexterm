          ...
          {step === 'email' && (
            <>
              {[
                ...
                <button key={s.label} style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:10, width:'100%', padding:'11px 16px', borderRadius:8, background:s.bg, border:`0.5px solid ${s.border}`, color:s.color, fontSize:14, fontWeight:600, cursor:'pointer', marginBottom:10, transition:'all 120ms', fontFamily:'inherit' }}
          ...
          <form onSubmit={handleContinue} style={{ display:'flex', flexDirection:'column', gap:14 }}>
            <div>
              ...
              <input
                ...
                placeholder="you@example.com" required autoFocus={step==='email'}
                style={inputStyle}
            ...
            <button type="submit" disabled={loading} style={{ padding:'12px 20px', borderRadius:8, background:loading?'rgba(240,180,41,.6)':'linear-gradient(135deg,#f0b429,#d4780a)', border:'none', color:'#000', fontSize:14, fontWeight:700, cursor:loading?'wait':'pointer', transition:'all 150ms', display:'flex', alignItems:'center', justifyContent:'center', gap:8, boxShadow:'0 2px 12px rgba(240,180,41,.25)' }}
              ...
              {loading ? (
                <>
                  <span style={{ width:14, height:14, borderRadius:'50%', border:'2px solid rgba(0,0,0,.3)', borderTopColor:'#000', animation:'t-spin .7s linear infinite', display:'inline-block' }} />
                  Signing in…
                </>
